...
import "../export-chart/export-chart.css";
...
const EditChart = () => {
  ...
  const [updatenewNodeName, setNewNodeName] = useState("");
  const [updatenewNodeTitle, setNewNodeTitle] = useState("");
  ...
  const onUpdateTitleChange = e => {
    setNewNodeTitle(e.target.value);
  };

  const updateNodes = async () => {
    await dsDigger.updateNodes([...selectedNodes].map(node => node.id), { id: uuidv4(), name: updatenewNodeName, title: updatenewNodeTitle });
    setDS({ ...dsDigger.ds });
  };
        ...
        <div className="new-nodes">
          <h4>Update Node</h4>
                  ...
                  <input
                    type="text"
                    placeholder="name"
                    value={updatenewNodeName}
                    onChange={onUpdateNameChange}
                  />
                  <input
                    type="text"
                    placeholder="title"
                    value={updatenewNodeTitle}
                    onChange={onUpdateTitleChange}
                  />                 
             <div className="updatenode" style={{padding:"1rem"}}>
